...
if st.button("Optimize Et"):
    ...
    if toplam_talep > toplam_kapasite:
        st.error("Toplam talep kapasiteyi aşıyor.")
        st.stop()
    ...
    col1.metric("Rastgele", f"{r_mesafe:.2f}")
    col2.metric("Optimize", f"{toplam_mesafe:.2f}")
    col3.metric("Tasarruf (%)", f"{tasarruf:.1f}")

    st.plotly_chart(fig, use_container_width=True)

    st.subheader("AI Değerlendirme")
    st.write(gemini_yorumla(arac_sayisi, toplam_mesafe, tasarruf, trafik))
